...
if "anon_uploads" not in st.session_state:
    st.session_state.anon_uploads = 0

# Cookie logic for anonymous session management
cookie_key = "anon_uploads"
cookie_anon_uploads = get_cookie(cookie_key)
if cookie_anon_uploads is not None:
    try:
        cookie_count = int(cookie_anon_uploads)
        st.session_state.anon_uploads = max(st.session_state.anon_uploads, cookie_count)
    except ValueError:
        pass
else:
    set_cookie(cookie_key, str(st.session_state.anon_uploads), max_age=3600*24)
...
def logout():
    st.session_state.logged_in = False
    st.session_state.user_email = ""
    set_cookie("anon_uploads", "0", max_age=0)  # Clear cookie
...
# Anonymous session gating
if not st.session_state.logged_in:
    if st.session_state.anon_uploads >= 3:
        st.warning("You’ve reached 3 submissions without login. Please log in or sign up to continue.")
        ...
        st.session_state.user_email = f"guest_{st.session_state.anon_uploads + 1}"
else:
    st.session_state.user_email = st.session_state.user_email or f"guest_{st.session_state.anon_uploads + 1}"
...
if st.session_state.user_email:
    ...
    if st.button("Submit All") and uploaded_file:
        if not st.session_state.logged_in and st.session_state.anon_uploads >= 3:
            st.warning("You've reached the maximum number of free uses. Please login or sign up to continue.")
            st.stop()
        ...
        bucket = "images" if st.session_state.logged_in else "testing"
        file_path = f"{user_id}/{uploaded_file.name}"

        try:
            res = supabase.storage.from_(bucket).upload(file_path, contents)
            if getattr(res, 'error', None):
                st.error(f"Failed to upload image: {res.error}")
                st.stop()
            image_url = f"{SUPABASE_URL}/storage/v1/object/public/{bucket}/{file_path}"
            ...
            if not st.session_state.logged_in:
                st.session_state.anon_uploads += 1
                set_cookie(cookie_key, str(st.session_state.anon_uploads), max_age=3600*24)
    ...
    if st.session_state.logged_in and st.button("Get Last Results"):
